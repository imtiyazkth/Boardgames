...
export default function LudoGame({ mode='solo', playerCount=4, playerNames=[], onExit, onGameOver }) {
  ...
  function toggleMusic(){
    audioEngine.unlock() // no-op if already unlocked; required inside a user gesture
    ...
    <div style={{
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            <button onClick={handleRoll} disabled={!canRoll||rolling}
              style={{
                ...
                boxShadow:diceVal===6&&!rolling?`0 0 26px 6px #ffd700cc`:canRoll?`0 6px 24px ${PC[currentPlayer]}70`:'none',
                transform:rolling?'rotate(25deg) scale(0.85)':canRoll?'scale(1.05)':'scale(1)',
                transition:'all 0.15s',
                animation:rolling?'none'
                  :diceVal===6?'lkSixSpin 0.7s cubic-bezier(.3,1.6,.4,1)'
                  :canRoll?'lkDiceWiggle 2s ease-in-out infinite':'none',
              }}>
              {diceVal&&!rolling
      ...
      <style>{`
        ...
        @keyframes lkSixSpin{
          ...
          50%{transform:rotate(360deg) scale(1.22)}
